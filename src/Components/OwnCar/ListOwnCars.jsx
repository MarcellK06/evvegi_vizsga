    ...
    $.ajax({
      ...
      success: function (resp) {
        ...
        resp.forEach((el) => {
          ...
          c.push(new OwnCar(el.id, car, el.vin, el.images, el.status, el.nickname, el.approved));
        });
        setOwnCars(c);
      },
    });
    ...
    <div className="row postcolor my-3 p-2 rounded">
      <div className="col-4">
        ...
        <div className="col-3 d-flex align-items-center justify-content-end">
          <p className={`fs-9 bold ${approved ? "text-success" : "text-danger"} m-0 mx-2`}>{approved ? "Ellenőrizve!" : "Ellenörzésre vár.." }</p>
        <FaEye size={25} className="mx-3 pointer" onClick={() => CreateModal(<div><p className="fs-3">{nickname}</p><hr /></div>, OwnCarEntry(el), true)} />
        <FaTrash size={25} className="mx-3 pointer" onClick={() => HandleDelete(el)}/>
        </div>
    </div>
    </>)
  }
